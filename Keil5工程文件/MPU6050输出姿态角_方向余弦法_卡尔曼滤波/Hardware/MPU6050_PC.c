...
// 利用加速度计计算pitch和roll的函数  
void ACC_calculateAngles(float ax, float ay, float az, float *pitch, float *roll) {  
    // 计算roll（横滚角），即绕Y轴的旋转  
    *roll = atan2f(ay,az) * (180.0 / pi); // 转换为度  

    // 计算pitch（俯仰角），即绕X轴的旋转  
    // 注意：当设备几乎平躺时（az接近0），atan2函数可能会产生不准确的结果  
    // 可以通过检查az的值并适当限制pitch的范围来避免这种情况   
    *pitch = atan2f(-ax, sqrt(ay * ay + az * az)) * (180.0 / pi); // 转换为度   
}  
